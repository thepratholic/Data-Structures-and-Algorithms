...
def solve():
    ...
    # dp[idx][cur_mod][tight]
    dp = [[[-1] * 2 for _ in range(d)] for _ in range(n)]

    def f(idx, cur_mod, tight):
        if idx == n:
            return 1 if cur_mod == 0 else 0
        
        if dp[idx][cur_mod][tight] != -1:
            return dp[idx][cur_mod][tight]
        ...
        for digit in range(ub + 1):
            ...
            new_cur_mod = (cur_mod + digit) % d

            ans += f(idx + 1, new_cur_mod, new_tight)
            ans %= MOD

        dp[idx][cur_mod][tight] = ans
        return dp[idx][cur_mod][tight]
    
    print(f(0, 0, True) - 1 + MOD % MOD)
